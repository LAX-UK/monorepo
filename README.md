...
3. `pnpm install`
4. `export DATABASE_URL=postgresql://postgres:postgres@localhost:5432/auction` then `pnpm db:migrate` (or `pnpm --filter @auction/db db:migrate:run` for `packages/db/src/migrate.ts`).
