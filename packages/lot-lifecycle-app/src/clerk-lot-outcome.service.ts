...
export class ClerkLotOutcomeService {
  constructor(
    ...
    private readonly notifications: ILotLifecycleNotifications,
    private readonly antiShillingGuard: IAntiShillingGuard | null = null,
    private readonly domainEventSink: ILifecycleDomainEventSink | null = null,
    private readonly lotLifecycleRecording: ILotLifecycleTransitionRecorder | null = null,
    /** Used only when voiding without a transition recorder (discouraged in production). */
    private readonly fallbackVoidEventProducer: string | null = null,
  ) {}
  ...
  async runFinalizeLotTransaction(
    ...
    return this.repos.runInTransaction(async ({ lot, bid }, tx) => {
      ...
      if (chosen?.placedByUserId && chosen.buyerLegalEntityId) {
        ...
        if (this.lotLifecycleRecording) {
        ...
        } else if (this.domainEventSink && this.fallbackVoidEventProducer) {
          await this.domainEventSink.withTx(tx).publish({
            ...
            schemaVersion: 1,
            producer: this.fallbackVoidEventProducer,
          });
        }
      }
