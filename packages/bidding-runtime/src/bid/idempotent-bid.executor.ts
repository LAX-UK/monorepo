...
import { IDEMPOTENCY_PENDING_VALUE } from "../ports.js";

/** Narrower than {@link IBidPlacer}: pins the concrete BidError so callers can branch on it. */
export interface IBidPlacerForIdempotency {
...
export class IdempotentBidExecutor {
  constructor(
    private readonly bidPlacer: IBidPlacerForIdempotency,
  ...
  async placeBidWithIdempotency(
    ...
    const trimmedBuyerLegalEntityId = input.buyerLegalEntityId?.trim();
    const buyerLegalEntityId: string = trimmedBuyerLegalEntityId
      ? trimmedBuyerLegalEntityId
      : (await this.legalEntityRepository.ensurePersonalEntity(placedByUserId)).id;
