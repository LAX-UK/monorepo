...
- **Admin tasks**: `source_of_funds_review` / `aml_screening_review` tasks are created on first escalation; completing them when MLRO decides is not automated yet — use the compliance queues as the system of record until task lifecycle is wired.
...
- **Triage then decide (two-stage)**: analyst triages via `POST /admin/compliance/source-of-funds/:id/triage`; MLRO/finance finalises via `POST /admin/compliance/source-of-funds/:id/decide`. An `approved` case satisfies the settlement gate (subject to validity above).
