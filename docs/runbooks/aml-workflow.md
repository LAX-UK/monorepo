...
- **In-platform document collection (staff-initiated)**: compliance officers request specific evidence types via `POST /admin/compliance/source-of-funds/:id/request-documents`. The buyer uploads on `/dashboard/compliance/source-of-funds` (status-only — no re-download). Files pass the standard presign → validate-upload pipeline plus a **ClamAV malware scan** for `source_of_funds_document` kinds; infected uploads are rejected. Staff download via short-TTL presigned GET (`aml.review` only) with a `source_of_funds.document_downloaded` audit event. Append-only `source_of_funds_document` rows preserve chain of custody (re-uploads supersede prior rows). After terminal decision, the worker emits buyer closure email + notification. Domain events: `documents_requested`, `document_uploaded`, `documents_submitted`, `document_downloaded`, then existing triage/decide events.

## Record retention

- Keep KYC artefacts and decision logs **5 years** after relationship end (typical MLR expectation — confirm with counsel).
- **Source-of-funds buyer documents**: tagged `retentionClass: aml_5y`. The worker job `purgeSourceOfFundsDocumentsJob` runs with GC and, **5 years after case resolution** (`reviewedAt` on approved/rejected cases), deletes the underlying upload object and anonymizes the document row (label nulled, `anonymizedAt` set). Append-only `domain_event` rows are retained for audit.
