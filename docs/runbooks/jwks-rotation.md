...
> **Status today.** The retirement helper [apps/worker/src/jobs/jwks-rotation.ts](../../apps/worker/src/jobs/jwks-rotation.ts) is implemented, but no scheduler invokes it. Quarterly rotation is performed manually using the procedure below until the BullMQ repeatable job ships.
...
6. **Delete the retired key.** `DELETE FROM jwks_key WHERE kid='<old_kid>' AND status='retired' AND rotated_at < now() - interval '30 minutes'`. Or run the `retireExpiredJwksKeys` helper from [apps/worker/src/jobs/jwks-rotation.ts](../../apps/worker/src/jobs/jwks-rotation.ts) which encodes the same predicate.
