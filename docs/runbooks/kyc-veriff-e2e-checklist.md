...
- [ ] With `STRICT_BID_ELIGIBILITY_ENABLED=true`, an authenticated user whose email is not verified sees **Email verification required** before any KYC state
...
- [ ] Organisation `admin`, `finance`, `owner`, and `buyer_agent` members are blocked unless the acting user has verified email and approved personal KYC; organisation KYB/status remains a separate gate
