...
- Start UTC: **2026-09-24T14:23:08Z** after [recovery 36007610661](https://github.com/LAX-UK/monorepo/actions/runs/36007610661) (**green**, including `shop_acceptance` and `record_accepted_release`). Test ingress routes `/commerce` to shop-identity; Identity SHA `7e86baadddbab4d3db42d93ceb90d309d510f049`. Repo vars `IDENTITY_SOAK_SHA_TEST` / `IDENTITY_SOAK_STARTED_AT_TEST` set to match.
- **Invalidated 2026-09-26:** chain stopped at 24h with ~82 in-window samples (late first dispatch + 16m cadence). Soak watch failed on post-window cron gaps until scripts bound the evaluator/watch to `[start, start+24h]`. Do **not** evaluate this window; re-baseline after test deploy of hosted-login merge (`ea4db252`) with Identity closure `ec3fd55` (lax-identity #22).
