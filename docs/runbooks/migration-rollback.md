...
| 0176 `shop_session_oidc_tokens` | Adds nullable encrypted OIDC token columns on `shop_identity_session` (`id_token_encrypted`, `refresh_token_encrypted`, `refresh_expires_at`). | `0176_rollback.sql` drops the three columns. | Encrypted ID/refresh material is lost; affected sessions must reauthenticate or complete silent upgrade after rollback code is live. | Roll back Shop Identity to a build that does not read/write encrypted tokens before SQL. Expect commerce 401s until users sign in again. |
