...
| 0162 `enable_oidc_end_session` | Adds the Better Auth trusted-RP end-session flag and enables it for Bid and Shop. | Drops the capability column. | Rolling back disables RP-initiated central logout; local RP sessions can still be cleared. | Roll back Shop/Bid logout callers before dropping the column. Apply only after `0161` with `PRODUCTION_MIGRATION_THROUGH=0162`. |
