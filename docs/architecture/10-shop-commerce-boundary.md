...
>
> - **Implemented in code:** `apps/shop-api` (Fastify v5 catalogue + commerce API), `packages/shop-domain`, `packages/shop-contracts`, Shop-owned Drizzle tables (catalogue + commerce: baskets, orders, reservations, Stripe checkout metadata, processed payment events, payout ledger), concurrency-safe idempotent import, public catalogue reads, basket/checkout/order HTTP routes, Stripe webhooks (raw body scoped to the webhook plugin only), and the Shop storefront (`apps/shop`) with server-side commerce calls to `apps/shop-identity` via `shopCommerceRequest`.
...
- **`apps/shop`** — server components call `shop-api` for catalogue reads via a runtime-validating client using `@auction/shop-contracts`. Commerce commands use `shopCommerceRequest` → `shopIdentityUrl` (server-to-server to `SHOP_IDENTITY_BASE_URL`), forwarding storefront cookies and applying upstream `Set-Cookie` through `applyUpstreamSetCookies` on server actions; Shop Identity exchanges (or uses the BFF token for guests) and calls `shop-api` basket/order routes.
