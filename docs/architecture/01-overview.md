...
> - **Scaffolded but not exercised:** `DomainEventPublisher` is wired into the container but no service calls `publish(tx, …)` yet. The Zoho/Xero projectors map and skip events but do not perform outbound HTTP. The Xero `projector_state` row is created but the runner only advances the `zoho` cursor. The JWKS retirement helper exists but is not scheduled. The `webhook-events` BullMQ queue has a consumer but no producer.
> - **Planned (not yet in repo):** Terraform/IaC, App Platform spec files, quarterly JWKS rotation cron, CSP/security-headers middleware, MFA, the v2 explicit account-link UX, and any Customer Account API integration with Shopify.
...
Three deployable units run on DigitalOcean App Platform. Each owns one concern. The boundary between them is enforced by what they're allowed to read from the database — the Postgres role split is the architectural firewall.
...
The async work engine. The intent is that every BullMQ consumer runs here: lot lifecycle, webhook processing, the Zoho and Xero projectors that read from `domain_events`, scheduled JWKS rotation, email sending, and image processing.

Today the directory exists at [apps/worker/](../../apps/worker/) with its own Dockerfile, env, Pino logger, Prometheus metrics, and graceful shutdown. It runs two BullMQ workers (queues `webhook-events` and the `domain-events` poller via [apps/worker/src/projectors/runner.ts](../../apps/worker/src/projectors/runner.ts)). What is **(Phase 2)** today: lot lifecycle still runs in `apps/api` (see above), the JWKS retirement helper is not scheduled, and the projectors are stubs — `mapDomainEventToZoho` returns labels and `ZohoClient.upsert` deliberately returns `{ ok: true }` without making outbound HTTP calls. Wiring the projectors to the real Zoho/Xero APIs is the next concrete work item.

The worker is the only app that horizontally scales. When Zoho is rate-limiting us during a hot auction, the operational lever is to scale `apps/worker` from 1 instance to N to drain the queue faster, without touching `apps/api` and without affecting the user-facing site. When Zoho recovers, scale back down. This is the single biggest operational lever the medium-grade tier gives us over the simple version, even though the outbound projection surface itself is **(Phase 2)**.
...
- More than three apps. The discipline is "no fourth app without written justification." Most "I want a new microservice" instincts are better solved as a new module inside `apps/worker`.
