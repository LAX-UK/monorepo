...
> - **JWKS retirement is now scheduled** by [packages/identity-db/src/adapters/drizzle-jwks-retirement.ts](../../packages/identity-db/src/adapters/drizzle-jwks-retirement.ts) running inside `apps/auth` under `auth_app`, with a Postgres advisory lock so only one auth replica performs each tick.
