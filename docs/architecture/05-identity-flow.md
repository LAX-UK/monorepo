...
3. **Client** — Primary sign-in is issuer-hosted (D27): `/login` and `/register` redirect through the Bid BFF; Google/Apple and phone run on `auth.lax.bid`. After callback, `PostLoginHandoff` runs `runHostedPostLoginEffects` (login analytics, sell-intent, `postAuthBroadcast`) before navigation.
