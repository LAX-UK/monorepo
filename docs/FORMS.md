...
## Bid eligibility controls

- Lot bid blockers reuse `SendVerificationEmailButton`; the verification
  callback returns to the attempted lot.
- `BidGate` owns pre-submit eligibility presentation across manual, auto-bid,
  full/compact sticky, and video-compact surfaces.
- Structured API codes (`email_not_verified`, `kyc_required`) drive recovery
  actions; user-facing message text is not the contract.
- Strict eligibility applies to the acting user for both personal and
  ...
  buyer-agent authorisation remain separate gates; Stripe Connect payout
  onboarding does not replace them.
