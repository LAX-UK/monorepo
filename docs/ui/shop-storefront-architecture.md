...
**Commerce mutations (basket, checkout, orders):** server actions and SSR basket reads call `shopCommerceRequest`, which server-to-server fetches `SHOP_IDENTITY_BASE_URL` commerce routes, forwards the incoming cookie jar, and applies upstream `Set-Cookie` (guest session, CSRF, basket token) to the storefront response via `applyUpstreamSetCookies`. Account chrome and health probes use the same `shopIdentityUrl` seam for read-only `/me` and dependency checks.
