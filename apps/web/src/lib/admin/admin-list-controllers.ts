...
export const artistsListController: IAdminListController<ArtistProfile, ArtistsListQuery> = {
  id: "artists",
  parseQuery(sp) {
    const base = parseListSearchParams(sp);
  ...
  async fetch(q) {
    const p: Parameters<typeof getAdminArtistList>[0] = {};
    if (q.q !== undefined && q.q !== "") p.q = q.q;
    if (q.includeArchived) p.includeArchived = true;
    const all = await getAdminArtistList(p);
