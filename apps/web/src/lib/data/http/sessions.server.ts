...
import type { MySessionsLoadResult, UserSessionRow } from "@/lib/data/user-session-row";
...
export async function getServerMySessions(): Promise<MySessionsLoadResult> {
  ...
  const body = (await res.json()) as { data: UserSessionRow[] };
  return { ok: true, sessions: body.data ?? [] };
}
