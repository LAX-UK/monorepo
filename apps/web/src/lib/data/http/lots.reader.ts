...
import type { ArchiveMetricsReader, ListLotsParams, LotReader } from "@/lib/data/contracts";
...
import {
  archiveEndedSummarySchema,
  bidEnvelopeSchema,
  lotCountBodySchema,
  lotDocumentPublicRowSchema,
  lotEnvelopeSchema,
  publicLotViewEnvelopeSchema,
  watchCountSchema,
} from "@/lib/data/http/lots.schema";
...
export const getServerLotBids = cache(async (lotId: string, limit = 50): Promise<Bid[]> => {
  ...
  const { rows } = readListEnvelope(body, bidEnvelopeSchema, `GET /lots/${lotId}/bids`);
  return rows;
});
...
export const getServerLotCount = cache(async (params: LotCountParams): Promise<number | null> => {
  try {
    ...
    const body = await readJsonBody(res);
    const parsed = lotCountBodySchema.safeParse(body);
    return parsed.success && typeof parsed.data.count === "number" ? parsed.data.count : null;
  } catch {
    return null;
  }
});
...
export const getServerLotWatchCount = cache(async (lotId: string): Promise<number> => {
  try {
    ...
    if (!res.ok) return 0;
    const body = await readJsonBody(res);
    const parsed = readDataEnvelope(body, watchCountSchema, `GET /lots/${lotId}/watch-count`);
...
export async function getServerArchiveMetricsReader(): Promise<ArchiveMetricsReader> {
  ...
  return {
    async getEndedSummary(endYear?: number) {
      ...
      const body = await readJsonBody(res);
      return readDataEnvelope(body, archiveEndedSummarySchema, "GET /lots/archive/summary");
    },
    async countEndedLots(filters: {
      ...
      const body = await readJsonBody(res);
      const parsed = lotCountBodySchema.parse(body);
...
async function listLotsViaCatalogueFetch(params: ListLotsParams): Promise<Lot[]> {
  ...
  const body = await readJsonBody(res);
  const { rows } = readListEnvelope(body, lotEnvelopeSchema, "GET /lots");
  return rows;
}
...
export async function getServerLotReader(): Promise<LotReader> {
  ...
  return {
    ...
    async getById(id: string): Promise<Lot | PublicLotView | null> {
      ...
      const body = await readJsonBody(res);
      return readDataEnvelope(body, publicLotViewEnvelopeSchema, `GET /lots/${id}`);
    },
  };
}
