...
function renderOrchestrator() {
  return render(<MarketingPromptOrchestrator enabled analytics={mocks.analytics} />);
}
...
describe("MarketingPromptOrchestrator", () => {
  beforeEach(() => {
    ...
    mocks.analytics.onImpression.mockReset();
    mocks.analytics.onDismiss.mockReset();
    mocks.analytics.onCta.mockReset();
  ...
  it("does not prompt on the first eligible page load", () => {
    renderOrchestrator();
    advance(45_000);
    expect(screen.queryByRole("dialog")).not.toBeInTheDocument();
    expect(mocks.analytics.onImpression).not.toHaveBeenCalled();
  });

  it("waits for active dwell and three eligible guest pages", () => {
    const view = renderOrchestrator();
    const rerender = () =>
      view.rerender(<MarketingPromptOrchestrator enabled analytics={mocks.analytics} />);
    ...
    expect(mocks.analytics.onImpression).toHaveBeenCalledWith(
      expect.objectContaining({ variant: "signup" }),
      "/artists",
    );
  });
  ...
  it("records selling intent and waits until a later eligible route", () => {
    ...
    expect(mocks.analytics.onCta).toHaveBeenCalledWith(
      expect.objectContaining({ trigger: "sell-content", variant: "selling" }),
      "/search",
    );
  });
  ...
  it("caps the session after dismissal", () => {
    ...
    expect(screen.queryByRole("dialog")).not.toBeInTheDocument();
    expect(mocks.analytics.onDismiss).toHaveBeenCalledOnce();

    navigate("/archive", rerender);
    advance(5_000);
    expect(screen.queryByRole("dialog")).not.toBeInTheDocument();
    expect(mocks.analytics.onImpression).toHaveBeenCalledOnce();
  });
});
