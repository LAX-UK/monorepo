...
describe("registerWorkerLotLifecycleConsumer (integration)", () => {
  it.skipIf(!redisUrl)("processes activate/end jobs via BullMQ", async () => {
    const url = redisUrl;
    if (!url) return;
  ...
  it.skipIf(!redisUrl)("retries failed end jobs until success", async () => {
    const url = redisUrl;
    if (!url) return;
    const redis = new Redis(url, { maxRetriesPerRequest: null });
    let endAttempts = 0;
  ...
  it.skipIf(!redisUrl)("wires attachDlq for exhausted lifecycle jobs", async () => {
    const url = redisUrl;
    if (!url) return;
    const redis = new Redis(url, { maxRetriesPerRequest: null });
    ...
    attachDlq(consumer.worker, LOT_LIFECYCLE_QUEUE_NAME, QUEUE_REGISTRY[LOT_LIFECYCLE_QUEUE_NAME], {
