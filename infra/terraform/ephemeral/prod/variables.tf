...
variable "email_from" {
  type        = string
  default     = "LAX.BID by London Art Xchange <no-reply@mail.lax.bid>"
...
variable "xero_redirect_uri" {
  type        = string
  default     = ""
  description = "OAuth redirect URI registered in Xero — must be the **web** app callback (e.g. https://lax.bid/admin/integrations/xero/callback), not the API host."
}
