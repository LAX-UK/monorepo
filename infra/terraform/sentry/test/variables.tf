...
variable "slack_channel_id" {
  type        = string
  default     = ""
  description = "Slack channel ID (C…). Optional — leave empty to skip Slack alerts until later; projects and DSNs still apply."
}

variable "pagerduty_service_name" {
  type        = string
  default     = "lax-primary"
  description = "PagerDuty service name in Sentry (unused in test — paging disabled)."
}
...
variable "pagerduty_integration_key" {
  ...
  default     = ""
  sensitive   = true
  description = "PagerDuty Events API integration key (unused in test)."
}
