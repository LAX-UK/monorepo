...
- `persistent/`: resources that survive teardown, including DNS, projects, and the shared `lax-media` Space.
- `sentry/`: isolated Sentry stack (projects, DSN keys, alerts, code mappings, notifications). State: `sentry-{env}/terraform.tfstate`.
...
4. Apply `sentry/prod` then `sentry/test` (requires Sentry integrations; see `BOOTSTRAP.md`).
5. Apply `ephemeral/test`.
6. After test has run for 24 hours, apply `ephemeral/prod`.
