...
export function parseRecoveryManifest(serialized) {
  ...
  if (
    manifest.version !== 1 ||
    typeof manifest.enableAuthSsfDelivery !== "boolean"
  ) {
    throw new Error(
      "Recovery manifest has an unsupported version or SSF state",
    );
  }
  ...
  requireString(
    manifest.migrationJournal?.sha256,
    "migrationJournal.sha256",
    /^[0-9a-f]{64}$/,
  );
